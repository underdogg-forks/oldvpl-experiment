    ...
    private function getInvoicesTotalDraft()
    {
        return CurrencyFormatter::format(InvoiceAmount::join('invoices', 'invoices.id', '=', 'invoice_amounts.invoice_id')
            ->whereHas('invoice', function ($q) {
                $q->draft();
                switch (config('ip.widget_invoice_summary_dashboard_totals')) {
                    case 'year_to_date':
                        $q->yearToDate();
                        break;
                    case 'this_quarter':
                    ...
                    case 'custom_date_range':
                        $q->dateRange(config('ip.widget_invoice_summary_dashboard_totals_from_date'), config('ip.widget_invoice_summary_dashboard_totals_to_date'));
                        break;
                }
    ...
    private function getInvoicesTotalPaid()
    {
        ...
        switch (config('ip.widget_invoice_summary_dashboard_totals')) {
            ...
            case 'this_quarter':
                ...
                break;
            case 'custom_date_range':
                $payments->dateRange(config('ip.widget_invoice_summary_dashboard_totals_from_date'), config('ip.widget_invoice_summary_dashboard_totals_to_date'));
                break;
        }
