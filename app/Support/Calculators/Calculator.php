        ...
        foreach ($this->items as $item) {
            ...
            if ($item['taxRatePercent']) {
                if (!$item['calculateVat']) {
                    $tax1 = round($discountedSubtotal * ($item['taxRatePercent'] / 100), config('ip.round_tax_decimals'));
                } else {
            ...
            if ($item['taxRate2Percent']) {
                if ($item['taxRate2IsCompound']) {
                    $tax2 = round(($discountedSubtotal + $tax1) * ($item['taxRate2Percent'] / 100), config('ip.round_tax_decimals'));
                } else {
                    $tax2 = round($discountedSubtotal * ($item['taxRate2Percent'] / 100), config('ip.round_tax_decimals'));
                }
            } else {
                $tax2 = 0;
            }
